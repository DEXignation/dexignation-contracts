...
2. `marketplace.buy(tokenId, maxPrice)` — in one transaction: check that the live listing price is at or below the buyer's maximum, pull USDC from the buyer, pay the seller (minus protocol fee), transfer the NFT to the buyer, and close the listing.
...
2. `marketplace.buy(tokenId, maxPrice)` — 한 트랜잭션에서: 실시간 리스팅 가격이 구매자의 최대 허용 가격 이하인지 확인, 구매자에게서 USDC 회수, 판매자에게 지급(프로토콜 수수료 차감), NFT를 구매자에게 이전, 리스팅 종료.
...
| `buy(tokenId, maxPrice)` | buyer | Atomically pay and receive the name, reverting if the live price exceeds `maxPrice` |
...
| `buy(tokenId, maxPrice)` | 구매자 | 실시간 가격이 `maxPrice`를 초과하면 revert하고, 아니면 원자적으로 결제하고 이름 수령 |
