...
이번 변경의 핵심 규칙은 하나다: **"판매된 서브네임은 만료 전까지 구매자의 것이다."**
...
| 2 | 판매된 서브네임이 **만료됨** | 잠금 자동 해제 → 부모가 회수 가능, 라벨 **재판매 가능** |
...
- **`isExpired`는 부모 만료를 상속**한다(부모 체인을 거슬러 올라가며 검사). 따라서 부모가 만료되면 자식도 만료로 간주되어 잠금 보호가 풀린다.
- 부모가 만료 후 정당하게 **회수(revoke)** 하면, 회수 시점에 잠금을 해제하여 이후 부모가 자유롭게 관리하도록 한다.
- 만료된 라벨을 부모가 무상 재발급(`issueSubnodeRecord`)하면, 이전 판매의 잔여 잠금을 정리한다.
...
모듈은 커머스 고유 책임(가격, 판매 토글, 기간, 접근 게이트, 수수료 분배, 위임 확인)만 담당한다.
...
1. alice.dex 소유자 Alice가 판매를 연다:
   - subnameRegistrar.configureSubname(aliceNode, price=1 POL, duration=1년, enabled=true)
...
```
1. team.alice.dex가 만료됨 (구매 시 설정한 duration 경과).
   - registry.isExpired(teamNode) == true → 잠금 보호 해제

2. Alice가 회수:
   - registry.revokeSubnodeRecord(aliceNode, "team", ...) → 성공
   - 소유권이 Alice로 복귀, subnodeSaleLocked = false (회수 시 해제)

3. 같은 라벨 "team"을 새 구매자에게 다시 판매 가능. ✓
```
...
| allows reclaim AFTER expiry | 만료 후 회수 성공 + 잠금 해제 |
...
**전체 스위트 결과:** `npm test` 기준 **207 passing**. 기존 `Subname-Issuance.test.ts`(무상 발급/회수/재지정 6개)와 `Marketplace.test.ts`(2LD 거래 17개)를 포함해 회귀 0건으로, 기존 동작이 보존됨을 확인했다.
